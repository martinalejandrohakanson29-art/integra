...
import { Search, UserPlus, Trash2, Edit3, Phone, User } from 'lucide-react';
...
const PatientListPage = () => {
  ...
  // 2. Función para borrar un paciente
      ...
      <div className="flex flex-col h-full space-y-4">
        ...
        <div className="relative max-w-md">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none dark:text-white"
          />
        </div>
          ...
          <div className="overflow-auto custom-scrollbar">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                {loading ? (
                  <tr><td colSpan="4" className="text-center py-10 text-slate-400">Cargando pacientes...</td></tr>
                  ...
                  <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col text-xs text-slate-500">
                        <span className="flex items-center gap-1"><Phone className="w-3 h-3" /> {p.telefono || 'Sin tel.'}</span>
                        <span>{p.email || ''}</span>
                      </div>
                    </td>
