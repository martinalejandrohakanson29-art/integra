      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center bg-slate-100 dark:bg-slate-700 rounded-lg p-1">
            <button onClick={() => setSelectedDate(subWeeks(selectedDate, 1))} className="p-1.5 hover:bg-white dark:hover:bg-slate-600 rounded-md transition-all">
            ...
            <button onClick={() => setSelectedDate(addWeeks(selectedDate, 1))} className="p-1.5 hover:bg-white dark:hover:bg-slate-600 rounded-md transition-all">
              <ChevronRight className="w-4 h-4 text-slate-600 dark:text-slate-300" />
            </button>
          </div>
          <button className="flex items-center gap-2 bg-[#527E95] hover:bg-opacity-90 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all shadow-sm">
        ...
        <div className="grid grid-cols-[80px_1fr_1fr_1fr_1fr_1fr] border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
          ...
          {days.map((day) => (
            <div key={day.toString()} className="p-3 text-center border-r border-slate-200 dark:border-slate-700 last:border-0">
              ...
              <p className={`text-lg font-bold ${format(day, 'yyyy-MM-dd') === format(startOfToday(), 'yyyy-MM-dd') ? 'text-indigo-600' : 'text-slate-700 dark:text-slate-200'}`}>
                {format(day, 'd')}
              </p>
            </div>
          ))}
