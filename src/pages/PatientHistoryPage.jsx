...
const PatientHistoryPage = () => {
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-950 font-sans selection:bg-[#517A91] selection:text-white">
            <Sidebar activePage="historia" />
            <main className="flex-1 flex flex-col overflow-hidden relative">
                <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 z-10 transition-colors">
                    <div className="flex items-center text-sm text-slate-500">
                        <span className="hover:text-[#517A91] cursor-pointer transition-colors" onClick={() => navigate('/pacientes')}>Pacientes</span>
                        <span className="material-symbols-outlined text-base mx-2">chevron_right</span>
                        <span className="font-medium text-slate-900 dark:text-white">Sofía Martínez</span>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            ...
                            <input className="pl-10 pr-4 py-2 bg-slate-100 dark:bg-slate-800 border-none rounded-full text-sm focus:ring-2 focus:ring-[#517A91] w-64 text-slate-900 dark:text-white" placeholder="Buscar registros..." type="text" />
                        </div>
                        <DarkModeToggle />
                    </div>
                </header>
                <div className="flex-1 overflow-y-auto p-6 md:p-8 scroll-smooth">
                    <div className="max-w-6xl mx-auto space-y-6">
                        <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                            <div className="flex items-center gap-6">
                                <div className="relative">
                                    <img alt="Sofía" className="w-20 h-20 rounded-full object-cover border-4 border-slate-50 dark:border-slate-800 shadow-md" src="https://lh3.googleusercontent.com/aida-public/AB6AXuD2D3QJMxRzljuROdy0KbzVaLBs6E2ySVG8XjLbQ-CAMVqi-NXqLXNB8wWdGKLIuSImgjaAYKqfhT5cebjcLOwuZdt4XAxt9SmQF_9uPeSMGKFUV8Stzlri8L2_yt9hCUEu1zxFbykE-wb5jqCPUHXFKIjloMGzom3Gxm4zS919iZGQVUZPvTabUz3EX2F7_q4gNDexa6QL_SFh8RG7SH7DnKNPhIJUTH2CJhLIOifhYN9ffPUM1PPam8dosG6UeV15GxjWwZS3KTI" />
                                ...
                                <div>
                                    <div className="flex items-center gap-3 mb-1">
                                        <h2 className="text-2xl font-bold font-quicksand text-slate-900 dark:text-white">Sofía Martínez</h2>
                                    ...
                                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-slate-500">
                                        <div className="flex items-center gap-1.5 bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded-lg border border-slate-100 dark:border-slate-700">
                                            ...
                                            <span className="font-bold">DNI:</span> <span className="text-slate-900 dark:text-white">45.123.890</span>
                                        </div>
                            ...
                            <div className="flex items-center gap-3 w-full md:w-auto">
                                <button className="flex-1 md:flex-none px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-medium hover:bg-slate-50 dark:hover:bg-slate-700 shadow-sm transition-colors">Editar Info</button>
                                <button className="flex-1 md:flex-none px-4 py-2.5 bg-[#517A91] text-white rounded-xl text-sm font-medium hover:bg-opacity-90 shadow-lg shadow-[#517A91]/20 transition-all">Nueva Cita</button>
                            </div>
                        </div>
                        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 min-h-[600px] flex flex-col">
                            <div className="border-b border-slate-200 dark:border-slate-800 px-2 md:px-6">
                                <nav className="flex space-x-2 md:space-x-8 overflow-x-auto">
                                    {['Información Personal', 'Historial de Tratamientos', 'Notas', 'Facturación'].map((tab, i) => (
                                        <a key={tab} href="#" className={`${i === 1 ? 'border-[#517A91] text-[#517A91]' : 'border-transparent text-slate-500 hover:text-slate-700'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors`}>{tab}</a>
                                    ))}
                                </nav>
                            </div>
                            <div className="p-6 md:p-8 flex-1">
                                <div className="flex items-center justify-between mb-8">
                                    <h3 className="text-lg font-bold text-slate-900 dark:text-white font-quicksand">Historial Médico</h3>
                                </div>
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                                            ...
                                            <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 hover:border-[#517A91]/30 transition-all">
                                                ...
                                                <div className="mt-3 flex gap-2">
                                                    <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-slate-700/50 text-slate-500 rounded-lg text-xs border border-slate-200 dark:border-slate-600">
