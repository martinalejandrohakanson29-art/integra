...
import { Calendar, Phone, Activity, ArrowLeft, FileText, X, Plus, Clock } from 'lucide-react';
...
const PatientHistoryPage = () => {
    ...
    // Estados para el Modal de Nueva Consulta
    ...
    const loadData = async () => {
        try {
            ...
            // Si el paciente tiene un odontograma base, lo cargamos en la nueva entrada por defecto
            if (found?.odontograma) {
                setNewEntry(prev => ({ ...prev, odontograma: found.odontograma }));
            }
        } catch (error) {
    ...
    const handleSaveConsulta = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setNewEntry({ ...newEntry, observaciones: '' });
                loadData();
            }
        } catch (error) {
            console.error(error);
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                        ...
                        <div className="divide-y divide-slate-100 dark:divide-slate-800">
                            {consultas.length === 0 ? (
                                ...
                                <div key={c.id} className="p-6 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                    ...
                                    <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                                        {c.observaciones || "Sin observaciones."}
                                    </p>
                                    {/* Aquí podríamos mostrar un resumen del odontograma si fuera necesario */}
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-slate-50 dark:bg-slate-950 w-full max-w-5xl max-h-[90vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-white/10">
                        <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
                            <div>
                                <h2 className="text-xl font-black text-slate-900 dark:text-white">Nueva Entrada Médica</h2>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="flex flex-col gap-2">
                                    <label className="text-xs font-black uppercase text-slate-400 tracking-tighter">Fecha de Consulta</label>
                                    <input 
                                        type="date" 
                                        value={newEntry.fecha}
                                        onChange={(e) => setNewEntry({...newEntry, fecha: e.target.value})}
                                        className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-white"
                                    />
                                </div>
                                <div className="md:col-span-2 flex flex-col gap-2">
                                    <label className="text-xs font-black uppercase text-slate-400 tracking-tighter">Observaciones y Tratamiento Realizado</label>
                                    <textarea 
                                        rows="3"
                                        placeholder="Describa el procedimiento realizado hoy..."
                                        value={newEntry.observaciones}
                                        onChange={(e) => setNewEntry({...newEntry, observaciones: e.target.value})}
                                        className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 text-slate-900 dark:text-white resize-none"
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                <label className="text-xs font-black uppercase text-slate-400 tracking-tighter">Actualizar Estado del Odontograma</label>
                                <div className="bg-white dark:bg-slate-900 p-2 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-x-auto">
                                    <Odontograma 
                                        data={newEntry.odontograma}
                                        onChange={(nuevaData) => setNewEntry({...newEntry, odontograma: nuevaData})}
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-end gap-4">
                            <button onClick={() => setIsModalOpen(false)} className="px-6 py-3 text-slate-500 font-bold hover:text-slate-700">Cancelar</button>
                            <button 
